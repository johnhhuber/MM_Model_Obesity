...
# specify the path in
path.in = '../output/trend_analysis/sensitivity_prior/'
...
for(pp in 1:length(n_knots_period))
{
  for(cc in 1:length(n_knots_cohort))
  {
    ...
    post <- read.csv(paste(path.in,'posterior_pooled_', n_knots_period[pp],'_',n_knots_cohort[cc],'.csv', sep = ''))
...
# best fitting model 3-6
which.min(df$BIC)


post <- read.csv('../output/trend_analysis/bmi_spline_0915/posterior_pooled_3_6.csv')
post_sens <- read.csv('../output/trend_analysis/sensitivity_prior/posterior_pooled_3_6.csv')

signif(apply(post_sens,2,quantile, probs = c(0.025,0.50,0.975)), digits = 2)
