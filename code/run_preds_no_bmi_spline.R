...
preds_no_bmi <- calc_preds_trend(post = post,
                          burn.in = 0:0,
                          num.samps = 200,
                          data_list = data_list,
...
preds_no_bmi_test <- calc_preds_trend(post = post,
                               burn.in = 0:0,
                               num.samps = 200,
...
preds_ho_bmi_combined <- calc_preds_trend(post = post,
                                   burn.in = 0:0,
                                   num.samps = 200,
...
# save output to file 
save(preds_no_bmi, file = paste(path.out, 'preds_', n_knots_period, '_', n_knots_cohort, '.RData', sep = ''))

# calculate the coverage probabilities 
sum(data_list$mm_trend_incid_male_nhw$c >= preds$x_male_nhw_ppi['2.5%',] & data_list$mm_trend_incid_male_nhw$c <= preds$x_male_nhw_ppi['97.5%',]) / length(data_list$mm_trend_incid_male_nhw$c)
sum(data_list$mm_trend_incid_female_nhw$c >= preds$x_female_nhw_ppi['2.5%',] & data_list$mm_trend_incid_female_nhw$c <= preds$x_female_nhw_ppi['97.5%',]) / length(data_list$mm_trend_incid_female_nhw$c)
sum(data_list$mm_trend_incid_male_nhb$c >= preds$x_male_nhb_ppi['2.5%',] & data_list$mm_trend_incid_male_nhb$c <= preds$x_male_nhb_ppi['97.5%',]) / length(data_list$mm_trend_incid_male_nhb$c)
sum(data_list$mm_trend_incid_female_nhb$c >= preds$x_female_nhb_ppi['2.5%',] & data_list$mm_trend_incid_female_nhb$c <= preds$x_female_nhb_ppi['97.5%',]) / length(data_list$mm_trend_incid_female_nhb$c)

sum(data_list_test$mm_trend_incid_male_nhw$c >= preds_test$x_male_nhw_ppi['2.5%',] & data_list_test$mm_trend_incid_male_nhw$c <= preds_test$x_male_nhw_ppi['97.5%',]) / length(data_list_test$mm_trend_incid_male_nhw$c)
sum(data_list_test$mm_trend_incid_female_nhw$c >= preds_test$x_female_nhw_ppi['2.5%',] & data_list_test$mm_trend_incid_female_nhw$c <= preds_test$x_female_nhw_ppi['97.5%',]) / length(data_list_test$mm_trend_incid_female_nhw$c)
sum(data_list_test$mm_trend_incid_male_nhb$c >= preds_test$x_male_nhb_ppi['2.5%',] & data_list_test$mm_trend_incid_male_nhb$c <= preds_test$x_male_nhb_ppi['97.5%',]) / length(data_list_test$mm_trend_incid_male_nhb$c)
sum(data_list_test$mm_trend_incid_female_nhb$c >= preds_test$x_female_nhb_ppi['2.5%',] & data_list_test$mm_trend_incid_female_nhb$c <= preds_test$x_female_nhb_ppi['97.5%',]) / length(data_list_test$mm_trend_incid_female_nhb$c)

sum(data_list_combined$mm_trend_incid_male_nhw$c >= preds_combined$x_male_nhw_ppi['2.5%',] & data_list_combined$mm_trend_incid_male_nhw$c <= preds_combined$x_male_nhw_ppi['97.5%',]) / length(data_list_combined$mm_trend_incid_male_nhw$c)
sum(data_list_combined$mm_trend_incid_female_nhw$c >= preds_combined$x_female_nhw_ppi['2.5%',] & data_list_combined$mm_trend_incid_female_nhw$c <= preds_combined$x_female_nhw_ppi['97.5%',]) / length(data_list_combined$mm_trend_incid_female_nhw$c)
sum(data_list_combined$mm_trend_incid_male_nhb$c >= preds_combined$x_male_nhb_ppi['2.5%',] & data_list_combined$mm_trend_incid_male_nhb$c <= preds_combined$x_male_nhb_ppi['97.5%',]) / length(data_list_combined$mm_trend_incid_male_nhb$c)
sum(data_list_combined$mm_trend_incid_female_nhb$c >= preds_combined$x_female_nhb_ppi['2.5%',] & data_list_combined$mm_trend_incid_female_nhb$c <= preds_combined$x_female_nhb_ppi['97.5%',]) / length(data_list_combined$mm_trend_incid_female_nhb$c)
