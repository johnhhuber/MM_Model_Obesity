...
mm_incid_trend_combined <- mm_incid_trend
mm_incid_trend_combined <- subset(mm_incid_trend_combined, year %in% 1975:2017)

yrs_train <- seq(from = 1975, to = 2019, by = 2)
yrs_test <- seq(from = 1976, to = 2016, by = 2)
mm_incid_trend_test <- subset(mm_incid_trend, year %in% yrs_test)
...
mm_incid_trend_test$x <- mm_incid_trend_test$x / 1e5
mm_incid_trend_test$x_lower <- mm_incid_trend_test$x_lower / 1e5
mm_incid_trend_test$x_upper <- mm_incid_trend_test$x_upper / 1e5
mm_incid_trend_test <- subset(mm_incid_trend_test, age_lower >= 40)

mm_trend_incid_male_nhw_test <- subset(mm_incid_trend_test, sex == 'Male' & race == 'White')
mm_trend_incid_female_nhw_test <- subset(mm_incid_trend_test, sex == 'Female' & race == 'White')
mm_trend_incid_male_nhb_test <- subset(mm_incid_trend_test, sex == 'Male' & race == 'Black')
mm_trend_incid_female_nhb_test <- subset(mm_incid_trend_test, sex == 'Female' & race == 'Black')

mm_incid_trend_combined$x <- mm_incid_trend_combined$x / 1e5
mm_incid_trend_combined$x_lower <- mm_incid_trend_combined$x_lower / 1e5
mm_incid_trend_combined$x_upper <- mm_incid_trend_combined$x_upper / 1e5
mm_incid_trend_combined <- subset(mm_incid_trend_combined, age_lower >= 40)

mm_trend_incid_male_nhw_combined <- subset(mm_incid_trend_combined, sex == 'Male' & race == 'White')
mm_trend_incid_female_nhw_combined <- subset(mm_incid_trend_combined, sex == 'Female' & race == 'White')
mm_trend_incid_male_nhb_combined <- subset(mm_incid_trend_combined, sex == 'Male' & race == 'Black')
mm_trend_incid_female_nhb_combined <- subset(mm_incid_trend_combined, sex == 'Female' & race == 'Black')
...
data_list_test = list(mgus_prev_male_nhw = mgus_prev_male_nhw,
                 mgus_prev_female_nhw = mgus_prev_female_nhw,
                 mgus_prev_male_nhb = mgus_prev_male_nhb,
                 mgus_prev_female_nhb = mgus_prev_female_nhb,
                 mm_incid_male_nhw = mm_incid_male_nhw,
                 ...
                 mm_incid_male_nhb = mm_incid_male_nhb,
                 mm_incid_female_nhb = mm_incid_female_nhb,
                 mm_trend_incid_male_nhw = mm_trend_incid_male_nhw_test,
                 mm_trend_incid_female_nhw = mm_trend_incid_female_nhw_test,
                 mm_trend_incid_male_nhb = mm_trend_incid_male_nhb_test,
                 mm_trend_incid_female_nhb = mm_trend_incid_female_nhb_test)

data_list_combined = list(mgus_prev_male_nhw = mgus_prev_male_nhw,
                      mgus_prev_female_nhw = mgus_prev_female_nhw,
                      mgus_prev_male_nhb = mgus_prev_male_nhb,
                      mgus_prev_female_nhb = mgus_prev_female_nhb,
                      mm_incid_male_nhw = mm_incid_male_nhw,
                      ...
                      mm_incid_male_nhb = mm_incid_male_nhb,
                      mm_incid_female_nhb = mm_incid_female_nhb,
                      mm_trend_incid_male_nhw = mm_trend_incid_male_nhw_combined,
                      mm_trend_incid_female_nhw = mm_trend_incid_female_nhw_combined,
                      mm_trend_incid_male_nhb = mm_trend_incid_male_nhb_combined,
                      mm_trend_incid_female_nhb = mm_trend_incid_female_nhb_combined)
...
weights_by_age_group_male_nhw_test <- list()
weights_by_age_group_female_nhw_test <- list()
weights_by_age_group_male_nhb_test <- list()
weights_by_age_group_female_nhb_test <- list()

for(ii in 1:nrow(data_list_test$mm_trend_incid_male_nhw))
{
  age_vec <- data_list_test$mm_trend_incid_male_nhw$age_lower[ii]:data_list_test$mm_trend_incid_male_nhw$age_upper[ii]
  pop_subset <- subset(pop_trend_male_nhw, year == data_list_test$mm_trend_incid_male_nhw$year[ii])
  pop_vec <- pop_subset$N[match(age_vec, pop_subset$age)]
  weights_by_age_group_male_nhw_test[[ii]] <- pop_vec / sum(pop_vec)
}
for(ii in 1:nrow(data_list_test$mm_trend_incid_female_nhw))
{
  age_vec <- data_list_test$mm_trend_incid_female_nhw$age_lower[ii]:data_list_test$mm_trend_incid_female_nhw$age_upper[ii]
  pop_subset <- subset(pop_trend_female_nhw, year == data_list_test$mm_trend_incid_female_nhw$year[ii])
  pop_vec <- pop_subset$N[match(age_vec, pop_subset$age)]
  weights_by_age_group_female_nhw_test[[ii]] <- pop_vec / sum(pop_vec)
}
for(ii in 1:nrow(data_list_test$mm_trend_incid_male_nhb))
{
  age_vec <- data_list_test$mm_trend_incid_male_nhb$age_lower[ii]:data_list_test$mm_trend_incid_male_nhb$age_upper[ii]
  pop_subset <- subset(pop_trend_male_nhb, year == data_list_test$mm_trend_incid_male_nhb$year[ii])
  pop_vec <- pop_subset$N[match(age_vec, pop_subset$age)]
  weights_by_age_group_male_nhb_test[[ii]] <- pop_vec / sum(pop_vec)
}
for(ii in 1:nrow(data_list_test$mm_trend_incid_female_nhb))
{
  age_vec <- data_list_test$mm_trend_incid_female_nhb$age_lower[ii]:data_list_test$mm_trend_incid_female_nhb$age_upper[ii]
  pop_subset <- subset(pop_trend_female_nhb, year == data_list_test$mm_trend_incid_female_nhb$year[ii])
  pop_vec <- pop_subset$N[match(age_vec, pop_subset$age)]
  weights_by_age_group_female_nhb_test[[ii]] <- pop_vec / sum(pop_vec)
}


weights_by_age_group_male_nhw_combined <- list()
weights_by_age_group_female_nhw_combined <- list()
weights_by_age_group_male_nhb_combined <- list()
weights_by_age_group_female_nhb_combined <- list()

for(ii in 1:nrow(data_list_combined$mm_trend_incid_male_nhw))
{
  age_vec <- data_list_combined$mm_trend_incid_male_nhw$age_lower[ii]:data_list_combined$mm_trend_incid_male_nhw$age_upper[ii]
  pop_subset <- subset(pop_trend_male_nhw, year == data_list_combined$mm_trend_incid_male_nhw$year[ii])
  pop_vec <- pop_subset$N[match(age_vec, pop_subset$age)]
  weights_by_age_group_male_nhw_combined[[ii]] <- pop_vec / sum(pop_vec)
}
for(ii in 1:nrow(data_list_combined$mm_trend_incid_female_nhw))
{
  age_vec <- data_list_combined$mm_trend_incid_female_nhw$age_lower[ii]:data_list_combined$mm_trend_incid_female_nhw$age_upper[ii]
  pop_subset <- subset(pop_trend_female_nhw, year == data_list_combined$mm_trend_incid_female_nhw$year[ii])
  pop_vec <- pop_subset$N[match(age_vec, pop_subset$age)]
  weights_by_age_group_female_nhw_combined[[ii]] <- pop_vec / sum(pop_vec)
}
for(ii in 1:nrow(data_list_combined$mm_trend_incid_male_nhb))
{
  age_vec <- data_list_combined$mm_trend_incid_male_nhb$age_lower[ii]:data_list_combined$mm_trend_incid_male_nhb$age_upper[ii]
  pop_subset <- subset(pop_trend_male_nhb, year == data_list_combined$mm_trend_incid_male_nhb$year[ii])
  pop_vec <- pop_subset$N[match(age_vec, pop_subset$age)]
  weights_by_age_group_male_nhb_combined[[ii]] <- pop_vec / sum(pop_vec)
}
for(ii in 1:nrow(data_list_combined$mm_trend_incid_female_nhb))
{
  age_vec <- data_list_combined$mm_trend_incid_female_nhb$age_lower[ii]:data_list_combined$mm_trend_incid_female_nhb$age_upper[ii]
  pop_subset <- subset(pop_trend_female_nhb, year == data_list_combined$mm_trend_incid_female_nhb$year[ii])
  pop_vec <- pop_subset$N[match(age_vec, pop_subset$age)]
  weights_by_age_group_female_nhb_combined[[ii]] <- pop_vec / sum(pop_vec)
}
...
bmi_prev <- data.frame(read.csv('../data/BMI/BMI_aggregated.csv'))
...
for(year in 1975:2018)
{
  ...
  for(ii in 1:nrow(bmi_prev_male_nhw_subset))
  {
    ...
    haz_male_nhw$haz_mgus_bmi[match(bmi_prev_male_nhw_subset$Age[ii], haz_male_nhw$age)] = 
    ...
    haz_female_nhw$haz_mgus_bmi[match(bmi_prev_female_nhw_subset$Age[ii], haz_female_nhw$age)] = 
    ...
    haz_male_nhb$haz_mgus_bmi[match(bmi_prev_male_nhb_subset$Age[ii], haz_male_nhb$age)] = 
    ...
    haz_female_nhb$haz_mgus_bmi[match(bmi_prev_female_nhb_subset$Age[ii], haz_female_nhb$age)] = 
    ...
    haz_male_nhw$haz_mm_bmi[match(bmi_prev_male_nhw_subset$Age[ii], haz_male_nhw$age)] = 
    ...
    haz_female_nhw$haz_mm_bmi[match(bmi_prev_female_nhw_subset$Age[ii], haz_female_nhw$age)] = 
    ...
    haz_male_nhb$haz_mm_bmi[match(bmi_prev_male_nhb_subset$Age[ii], haz_male_nhb$age)] = 
    ...
    haz_female_nhb$haz_mm_bmi[match(bmi_prev_female_nhb_subset$Age[ii], haz_female_nhb$age)] = 
...
save(data_list,
     ...
     weights_by_age_group_male_nhw_test,
     weights_by_age_group_male_nhb_test,
     weights_by_age_group_female_nhw_test,
     weights_by_age_group_female_nhb_test,
     weights_by_age_group_male_nhw_combined,
     weights_by_age_group_male_nhb_combined,
     weights_by_age_group_female_nhw_combined,
     weights_by_age_group_female_nhb_combined,
     ages,
